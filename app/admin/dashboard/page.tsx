...
const menuItems: MenuItem[] = [
  ...
  { key: "pending-admins", label: "Pending Admins", icon: UserCheck },
...
export default function AdminDashboardPage() {
  ...
  const visibleMenuItems = menuItems.filter((item) => {
    if (item.key === "pending-admins" && adminProfile?.role !== "super_admin") {
      return false;
    }
    return true;
  });
  ...
  useEffect(() => {
    ...
    const handlePushSignal = useCallback(async (forcedAction?: "buy" | "sell") => {
    if (!forcedAction) {
  setSignalMessage("Please choose Buy or Sell.");
  return;
}
    if (!signalEA || !signalSymbol.trim()) {
      ...
      switch (item.key) {
       ...
       case "generate":
        ...
        case "profile":
          ...
          return;
        case "pending-admins":
  router.push("/admin/pending-admins");
  return;
        case "stats":
        case "push-signals":
        case "control-bot":
...
{activeTab === "dashboard" && (
  ...
  <span className="bg-gradient-to-r from-cyan-300 via-sky-400 to-blue-500 bg-clip-text text-transparent">
    {adminName}
          ...
          <p className="mt-3 max-w-2xl text-sm leading-7 text-white/62 sm:text-base">
            Manage your licenses, Expert Advisors, users, and signals from your personalized control center.
          </p>
  ...
  >
    {sendingSignal ? "Sending..." : "Buy"}
  </button>

  <button
    type="button"
    onClick={() => handlePushSignal("sell")}
    disabled={sendingSignal}
    className="rounded-2xl border border-red-300/20 bg-red-400/10 px-5 py-3 text-sm font-semibold text-red-300 transition hover:bg-red-400/15 disabled:opacity-60"
  >
    {sendingSignal ? "Sending..." : "Sell"}
  </button>
</div>
                </div>
              </div>
